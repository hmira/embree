...
namespace embree
{
  ...
  Col3f c; Sample3f wi;BRDFType type; float ss;
...
//       float ss = sampler->getFloat(firstScatterTypeSampleID + lightPath.depth);
	...
	if ( wi.pdf > 0.0f )
	{
		...
		if ( diff.light  && dot( diff.Ng, r.dir ) < 0 )
		{
		  ...
		  lightpdf = diff.light->pdf( dg, wi );
		}
		
  		weight = mis(wi.pdf, lightpdf);
		
	if ( dot( diff.Ng, r.dir ) < 0 
	 && (type != GLOSSY_REFLECTION )  
	)	
	...
	/* LIGHTSAMPLING */
      ...
      ss = sampler->getFloat(firstScatterTypeSampleID + lightPath.depth);//TERAZ PRIDANE
      
   	weight = mis( ls.wi.pdf, brdfs.pdf( wo, dg, wi, ss, giBRDFTypes ) );
        /*! Evaluate BRDF. */
